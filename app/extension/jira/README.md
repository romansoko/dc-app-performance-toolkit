- Make sure you have a few ws issues ready before running extension tests.
- jql to filter ws issues:    
    reporter = WS_BOT   
    summary ~ 'Whitesource*
----
 YAML FILE USED - DON'T FORGET TO FILL CREDENTIALS 
---
```
settings:
  ...
  verbose: false
  env:
    application_hostname: jira-dev.whitesourcedev.com    # Jira DC hostname without protocol and port e.g. test-jira.atlassian.com or localhost
    ...
    test_duration: 5m
    ramp-up: 3s                     # time to spin all concurrent users
    ...
    view_backlog: 6
    browse_boards: 1
    standalone_extension: 15 # By default disabled
    # Custom dataset section.
    custom_dataset_query: "summary ~ 'Whitesource*'"          # Write JQL query to add JQL output to the app/datasets/jira/custom-issues.csv, e.g. "summary ~ 'AppIssue*'"
services:
  - module: shellexec
    prepare:
